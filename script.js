    ...
    // carregar J SON :P
    async loadSkillTree() {
        const res = await fetch("skilltree.json");
        const data = await res.json();

        this.ascensionUpgrades = data.map(u => ({
            ...u,
            owned: false
        }));
    },
    ...
    canBuy(u) {
        ...
        if (this.ascensionPoints < u.cost) return false;
    ...
    applyAscensionBonuses() {
        ...
        this.ascensionUpgrades.forEach(u => {
            if (u.owned) {
                this.clickPower += u.power || 0;
                this.autoClicks += u.auto || 0;
        ...
        // upgrades normais
        this.upgrades.forEach(u => {
            this.clickPower += u.powerInc * u.owned;
        ...
        patchNotes: [
            ...
            new Patch("Ascenção util + novo site", "06/04/2025", " AAA")
        ],
...
`Poder: ${this.clickPower} | Auto: ${this.autoClicks.toLocaleString('pt-BR', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} | Intervalo: ${this.attackInterval.toLocaleString('pt-BR', { minimumFractionDigits: 1, maximumFractionDigits: 1 })}ms | DPS: ${(this.autoClicks / (this.attackInterval / 1000)).toLocaleString('pt-BR', { minimumFractionDigits: 1, maximumFractionDigits: 1 })}/s`;            this.applyAscensionBonuses()
    ...
    renderAscension() {
        ...
        this.ascensionUpgrades.forEach(u => {
            ...
            el.style.gridColumn = u.x;
            el.style.gridRow = u.y + 1;
            ...
            el.classList.remove("locked", "available", "bought");

            if (u.owned) {
                el.classList.add("bought");
            } else if (this.canBuy(u)) {
...
requestAnimationFrame(gameLoop);
    ...
    function saveGame() {
        localStorage.setItem('save', JSON.stringify({
            ...
            ascensionUpgrades: game.ascensionUpgrades.map(u => ({
            id: u.id,
            owned: u.owned
            }))
        }));
    ...
    function loadGame() {  //FLOAD
        ...
        data.ascensionUpgrades.forEach(saved => {
            const up = game.ascensionUpgrades.find(u => u.id === saved.id);
            if (up) {
                up.owned = saved.owned;
            }
        });
    ...
    async function loadEnemies() {
        try {
            ...
            console.log("Enemies carregados:", enemyData); // Debugguer :P
        } catch (err) {
